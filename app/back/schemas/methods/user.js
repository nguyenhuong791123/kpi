...
async function addUserMethods(fields, flag) {
    const UserType = await new Promise(async (resolve, reject) => { return await InitPromise.then(async function(obj){ resolve(await obj["User"]) }) });
    if(flag) {
        fields['getUser'] = {
            type: UserType,
